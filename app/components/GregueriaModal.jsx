  ...
  render() {
    ...
    if (this.props.gregueria.tags.length == 0) {
      ...
      tags = this.props.gregueria.tags.map((tag, i) => (
        <span key={i}>
          <LinkContainer to={"/topics?id=" + tag + "#"}>
    ...
    const similarGreguerias = this.props.similar.map(gregueria => {
      return (
        <Gregueria gregueria={gregueria} onBodyClick={this.props.onBodyClick} />
      );
    });

    return (
      <div>
        <div className="gregueria-header" onClick={this.props.onClose}>
          Back
        </div>
        <div className="gregueria-content">
          ...
          <div className="gregueria-similar">{similarGreguerias}</div>
        </div>
      </div>
    );
  }
