...
export default class GregueriasMap extends React.Component {
  ...
  componentDidMount() {
    ...
    x.domain(
      d3.extent(this.props.greguerias, point => {
        return point.x;
      })
    ).nice();
    y.domain(
    ...
    $(".map").mousemove(event => {
      ...
      $(".map-focus")
        .css({ top: event.pageY + cardOffset, left: event.pageX + cardOffset })
        .text(this.props.greguerias[closest.id].text)
        .show();
  ...
  componentWillReceiveProps(nextProps) {
    this.setState(
      {
        results: nextProps.results.map(result => {
          return result.id;
        })
      },
      () => {
  ...
  draw(transform) {
    ...
    for (let i = 0; i < this.props.greguerias.length; ++i) {
      ...
      if (
        point.id == this.state.selectedPoint ||
        this.state.results.includes(point.id)
      )
        continue;
